...
model.load_weights('weights/classification_weights.176-0.82.hdf5')
...
'''
model2 = resnet_v1.resnet_v1(input_shape=(448,448,3), num_classes=7, depth=20, attention_module='cbam_block')
model2.compile(loss=weighted_categorical_crossentropy([4.375, 2.783, 1.301, 12.440, 1.285, 0.213, 10.075]),
              optimizer=Adam(lr=0.002),
              metrics=['accuracy'])
model2.load_weights('weights/cifar10_resnet20_cbam_block_model.183.h5')
'''



# average ensemble model1 + model2
...
val_generator.reset()
print("-- Evaluate --")
scores = model3.evaluate_generator(val_generator, steps=np.ceil(float(val_generator.samples) / float(batch_size)))
print("%s: %.2f%%" %(model3.metrics_names[1], scores[1]*100))

val_generator.reset()
predictions = model3.predict_generator(val_generator, steps = np.ceil(float(val_generator.samples)/float(batch_size)))
...
val_generator.reset()
print("-- Evaluate --")
scores = model4.evaluate_generator(val_generator, steps=np.ceil(float(val_generator.samples) / float(batch_size)))
print("%s: %.2f%%" %(model4.metrics_names[1], scores[1]*100))

val_generator.reset()
predictions = model4.predict_generator(val_generator, steps = np.ceil(float(val_generator.samples)/float(batch_size)))
...
val_generator.reset()
print("-- Evaluate --")
scores = ensemble_model.evaluate_generator(val_generator, steps=np.ceil(float(val_generator.samples) / float(batch_size)))
print("%s: %.2f%%" %(ensemble_model.metrics_names[1], scores[1]*100))

val_generator.reset()
predictions = ensemble_model.predict_generator(val_generator, steps = np.ceil(float(val_generator.samples)/float(batch_size)))
